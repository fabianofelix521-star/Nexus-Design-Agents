...
execFileSync(process.execPath, [cliEntry, 'export', '--brief', briefPath, '--target', 'openclaw', '--output', outDir], {
  cwd: workspaceRoot,
  stdio: 'inherit',
});
...
const adapter = new RuntimeAdapterRegistry().resolve('openclaw');

if (!adapter) {
  throw new Error('OpenClaw adapter was not registered.');
}

const handshake = {
  request: adapter.createHandshake(bundle),
  response: adapter.negotiate(bundle, ['multi-agent', 'tool-use', 'code-editing']),
};

await writeFile(path.join(outDir, 'handshake-result.json'), `${JSON.stringify(handshake, null, 2)}\n`, 'utf8');
process.stdout.write(`OpenClaw runtime demo written to ${outDir}\n`);